...
function addShakeHorizontal(elem) {
...
form.addEventListener('submit', function(event) {
    const dataSend = form.getAttribute('data-send');
    ...
    if (!selectInput.value) {
        event.preventDefault();
        addShakeHorizontal(selectInput);
        return false
    }
    if (!validateEmail(emailInput.value.trim())) {
        event.preventDefault();
        addShakeHorizontal(emailInput);
        return false
    }
    if (dataSend === 'true') {
        event.preventDefault();
        openModal(contentModal.formEnd);
        form.reset();
        return false
    }
    event.preventDefault();
    form.reset();
    form.setAttribute('data-send', 'true');
